...
class AcfRepeater extends \DynamicContentForElementor\Widgets\WidgetPrototype
{
    ...
    {
        $settings = $this->get_settings_for_display();
        $icon = $this->get_settings_for_display('selected_icon');
        $icon_active = $this->get_settings_for_display('selected_active_icon');
        $icon_align = $this->get_settings_for_display('selected_icon_align') ?? 'left';
        if (!empty($icon)) {
