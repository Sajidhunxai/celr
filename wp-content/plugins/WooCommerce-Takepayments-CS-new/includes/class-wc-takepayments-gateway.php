...
class WC_Payment_Network extends WC_Payment_Gateway
{
	...
	public function __construct()
	{
		...
		$this->default_gateway     = $configs['default_gateway'];
		...
		if ($this->settings['showAmexLogo'] === 'Y') {
            $this->icon = plugins_url('/', dirname(__FILE__)) . 'assets/img/logo-amex.png';
        }
	...
	public function init_form_fields()
	{
		$this->form_fields = array(
			...
			'gatewayURL' => array(
				...
				'type'        => 'text',
				'description' => __('Please enter your gateway URL. (Only enter the Base URL without sub-directory!)', $this->lang),
				'default'     => $this->default_gateway,
			...
			'showAmexLogo' => array(
                'title'       => __('Show American Express Card Logo', $this->lang),
                'type'        => 'select',
                'options' => array(
                    'N'       => 'No',
                    'Y'       => 'Yes'
                ),
                'description' => __('If your account has the use of the American Express Card enabled you can show this at the check out.', $this->lang),
                'default'     => 'No'
            ),
	...
	public function process_payment($order_id)
	{
		...
		if (in_array($this->settings['type'], ['hosted', 'hosted_v2', 'hosted_v3', 'iframe'], true)) {
			return array(
				'result'   => 'success',
	...
	{
		if (in_array($this->settings['type'], ['hosted', 'hosted_v2', 'hosted_v3', 'iframe'])) {
			...
			if($this->settings['type'] === 'iframe'){
                wp_enqueue_script('tp-modal',"https://gw1.tponlinepayments.com/sdk/web/v1/js/hostedforms.min.js", array('jquery'), "1.0", false );
                $iframe_payment = '<style>form#paymentForm{text-align:center;};#takepayments-iframe-paynow{background:red;}</style>
                <div id="iframeTarget"></div>
                <script> let req ={';
                foreach($req as $key =>$value)
                {
                    $iframe_payment .= $key .":'".  $value ."',\n";
                }
                $iframe_payment .= "signature:'".Gateway::sign($req,$this->settings['signature'])."'};
                // console.log(req);
                let data = {
                 id: 'paymentForm',
                url: 'https://gw1.tponlinepayments.com/hosted/modal/',
                // url: 'https://gw1.tponlinepayments.com/paymentform/',
        
                modal: true,
                data: req,
                submit: {
                id: 'takepayments-iframe-paynow',
                type: 'button',
                label: 'Pay With Takepayments',
                    
                }};
                let form = new window.hostedForms.classes.Form('iframeTarget',data); 
                let formBtn = document.getElementById('takepayments-iframe-paynow');
                let formBtnStyles ='button#takepayments-iframe-paynow{background-color: #b20b63;color:#fff;border-color:#b20b63;border-radius:100px;font-weight:bold;font-size:1rem;transition:color .15s ease-in-out,background-color .15s ease-in-out,border-color .15s ease-in-out,box-shadow .15s ease-in-out;}button#takepayments-iframe-paynow:hover{color: #fff;background-color: #8e094f;border-color: #8e094f}';
                let formBtnStylesElement = document.createElement('style');
                formBtnStylesElement.styleSheet ?  formBtnStylesElement.styleSheet.cssText = formBtnStyles : formBtnStylesElement.appendChild(document.createTextNode(formBtnStyles));
                document.getElementsByTagName('head')[0].appendChild(formBtnStylesElement);
                </script>";
                echo $iframe_payment;
            }else{
	...
	public function on_order_success($response)
	{
		...
		if(isset($_SESSION['threeDSRef'])){unset($_SESSION['threeDSRef']);}
	...
	public function process_response_callback($response = null)
	{
		...
		if (isset($_POST['threeDSMethodData']) || isset($_POST['cres'])) {
			...
			$req = array(
				...
				'threeDSRef' => isset($_COOKIE['threeDSRef']) ? $_COOKIE['threeDSRef'] : $_SESSION['threeDSRef'] ,
	...
	protected function capture_order($order_id)
	{
		...
		if ($this->settings['customerWalletsEnabled'] === 'Y' && is_user_logged_in()) {
			...
			global $wpdb;
			$wallet_table_name = $wpdb->prefix . 'woocommerce_takepayments_wallets';
	...
	protected function create_wallet($response)
	{
		...
		if ($this->settings['customerWalletsEnabled'] === 'Y' && isset($response['walletID']) && $order->get_user_id() != 0) {
			$wallet_table_name = $wpdb->prefix . 'woocommerce_' . 'takepayments_' . 'wallets';
